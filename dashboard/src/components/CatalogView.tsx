        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <div style={{
            ...
            <input
              ...
              style={{ background: 'transparent', border: 'none', color: 'var(--text-primary)', fontSize: '13px', outline: 'none', width: '200px' }}
            />
          </div>
        </div>
      </div>
          ...
          return (
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                <Store size={20} color={isSelected ? '#d97706' : '#9CA3AF'} />
                <span className="badge badge-gold">
                  <Star size={11} /> {parseFloat(String(m.rating)).toFixed(2)}
                </span>
              </div>
              <h4 style={{ fontSize: '15px', color: 'var(--text-primary)', margin: '0 0 4px 0' }}>{m.name}</h4>
          ...
          filteredCatalog.map((p, idx) => (
            <div key={idx} className="glass-panel" style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div>
                  <h4 style={{ fontSize: '15px', color: 'var(--text-primary)', margin: 0 }}>{p.canonical_name}</h4>
                  <span style={{ fontSize: '12px', color: 'var(--text-tertiary)' }}>{p.name_ar}</span>
                </div>
